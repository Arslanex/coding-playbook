...
SCOPE: how the slug gets into the path and the API. MUST NOT: `features/tenant/` as a second backbone. MUST NOT [critical]: tenant from JWT in the browser (12).
...
`features/tenants` — switcher, list memberships. Other features receive `slug` as a prop/param and send it to FastAPI the way the backend Extra says (header or path the API documents). MUST [critical]: the **API** filters; hiding a nav item is not isolation (15).
