...
MUST NOT: `http/` import infra except `http/deps` opening `infra/db/session.py`, and `http/` cache calls for rate-limit / idempotency / `jti` denylist (10, [13-identity-security.md](13-identity-security.md)).
