...
Tenant is a product noun (the customer org). It is not a mixin you sprinkle on day one of Extra, and it is not `core/`.
...
MUST NOT: `core/tenant.py` toolbox. MUST NOT: RLS as the **only** gate — application filter stays primary (15). MUST NOT: `modules/common/` for "tenant helpers." Other modules call `TenantService` or receive `tenant_id` from deps into **their** service.
